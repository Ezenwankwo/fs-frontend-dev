<template>
    <form action="" class="frm PID tradeP2P" @submit.prevent="notifySeller">
        ...
        <span class="process">
            <div class="process-position">
                ...
                <button type="submit" class="process-item process-forward">Save & continue</button>
            </div>
        </span>
    </form>
</template>
...
const wallet = useConversionStore().$state.wallet
const account = useConversionStore().$state.receivingAccount
...
const notifySeller = () => {
    const res = useFetch(
        `${config.public.baseURL}/trades/notify-seller/`,
        {
            'method': 'post',
            'body': {
                "trade_id": trade.public_id,
